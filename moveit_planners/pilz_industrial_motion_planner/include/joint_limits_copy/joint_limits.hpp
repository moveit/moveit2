...
struct JointLimits
{
  ...
  std::string to_string()  // NOLINT: Ignore case style
  {
    ...
    {
      ss_output << "  velocity limit: " << '[' << max_velocity << "]\n";
    }
    if (has_acceleration_limits)
    {
      ss_output << "  acceleration limit: " << '[' << max_acceleration << "]\n";
    }
    if (has_jerk_limits)
    {
      ss_output << "  jerk limit: " << '[' << max_acceleration << "]\n";
    }
    if (has_effort_limits)
    {
      ss_output << "  effort limit: " << '[' << max_acceleration << "]\n";
    }
    if (angle_wraparound)
    {
