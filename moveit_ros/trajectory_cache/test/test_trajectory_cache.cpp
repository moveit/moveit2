...
void check_and_emit(bool predicate, const std::string& test_case, const std::string& label)
{
  if (predicate)
  {
    std::cout << "[PASS] " << test_case << ": " << label << std::endl;
  }
  else
  {
    std::cout << "[FAIL] " << test_case << ": " << label << std::endl;
  }
}
...
void test_getters_and_setters(std::shared_ptr<TrajectoryCache> cache)
{
  std::string test_case = "getters_and_setters";

  check_and_emit(cache->getDbPath() == ":memory:", test_case, "getDbPath");
  check_and_emit(cache->getDbPort() == 0, test_case, "getDbPort");

  check_and_emit(cache->getExactMatchPrecision() == 10, test_case, "getExactMatchPrecision");
  cache->setExactMatchPrecision(1);
  check_and_emit(cache->getExactMatchPrecision() == 1, test_case, "getExactMatchPrecisionAfterSet");

  check_and_emit(cache->getNumAdditionalTrajectoriesToPreserveWhenDeletingWorse() == 10, test_case,
                 "getNumAdditionalTrajectoriesToPreserveWhenDeletingWorse");
  cache->setNumAdditionalTrajectoriesToPreserveWhenDeletingWorse(1);
  check_and_emit(cache->getNumAdditionalTrajectoriesToPreserveWhenDeletingWorse() == 1, test_case,
...
void test_motion_trajectories(std::shared_ptr<MoveGroupInterface> move_group, std::shared_ptr<TrajectoryCache> cache)
{
  ...
  // Initially empty
  test_case = "test_motion_trajectories.empty";

  check_and_emit(cache->countTrajectories(ROBOT_NAME) == 0, test_case, "Trajectory cache initially empty");

  check_and_emit(cache->fetchAllMatchingTrajectories(*move_group, ROBOT_NAME, plan_req, 999, 999).empty(), test_case,
                 "Fetch all trajectories on empty cache returns empty");

  check_and_emit(cache->fetchBestMatchingTrajectory(*move_group, ROBOT_NAME, plan_req, 999, 999) == nullptr, test_case,
  ...
  test_case = "test_motion_trajectories.put_second.fetch_matching_no_tolerance";
  ...
  check_and_emit(fetched_trajectories.size() == 2, test_case, "Fetch all returns two");
  check_and_emit(fetched_traj != nullptr, test_case, "Fetch best trajectory is not nullptr");

  check_and_emit(*fetched_trajectories.at(0) == *fetched_traj, test_case, "Fetched trajectory on both fetches match");

  check_and_emit(*fetched_traj == traj, test_case, "Fetched trajectory matches original");

  check_and_emit(fetched_traj->lookupDouble("execution_time_s") == execution_time, test_case,
  ...
  test_case = "test_motion_trajectories.put_second.fetch_non_matching_out_of_tolerance";
  ...
  check_and_emit(fetched_trajectories.size() == 0, test_case, "Fetch all returns empty");
  check_and_emit(fetched_traj == nullptr, test_case, "Fetch best trajectory is nullptr");
  ...
  //
  // Non-matching key should not have cache hit
  test_case = "test_motion_trajectories.put_second.fetch_non_matching_only_goal_in_tolerance";
  ...
  test_case = "test_motion_trajectories.put_second.fetch_swapped";
  ...
  test_case = "test_motion_trajectories.put_second.fetch_smaller_workspace";
  ...
  test_case = "test_motion_trajectories.put_second.fetch_larger_workspace";
  ...
  test_case = "test_motion_trajectories.put_better.fetch_sorted";
  ...
  check_and_emit(fetched_trajectories.size() == 3, test_case, "Fetch all returns three");
  check_and_emit(fetched_traj != nullptr, test_case, "Fetch best trajectory is not nullptr");
  ...
  check_and_emit(fetched_traj->lookupDouble("execution_time_s") == better_execution_time, test_case,
                 "Fetched trajectory has correct execution time");
  ...
  check_and_emit(fetched_trajectories.size() == 1, test_case, "Fetch all returns one");
  check_and_emit(fetched_traj != nullptr, test_case, "Fetch best trajectory is not nullptr");

  check_and_emit(*fetched_trajectories.at(0) == *fetched_traj, test_case, "Fetched trajectory on both fetches match");

  check_and_emit(*fetched_traj == different_traj, test_case, "Fetched trajectory matches original");

  check_and_emit(fetched_traj->lookupDouble("execution_time_s") == even_better_execution_time, test_case,
                 "Fetched trajectory has correct execution time");
  ...
  //
  // With multiple trajectories in cache, fetches should be sorted accordingly
  test_case = "test_motion_trajectories.put_different_req.fetch";
  ...
  test_case = "test_motion_trajectories.different_robot.empty";
  ...
  check_and_emit(cache->countTrajectories(ROBOT_NAME) == 2, test_case, "Two trajectories in original robot's cache");

  fetched_trajectories = cache->fetchAllMatchingTrajectories(*move_group, ROBOT_NAME, different_plan_req, 0, 0);

  check_and_emit(fetched_trajectories.size() == 1, test_case, "Fetch all on original still returns one");
}
...
void test_cartesian_trajectories(std::shared_ptr<MoveGroupInterface> move_group, std::shared_ptr<TrajectoryCache> cache)
{
  ...
  test_case = "test_cartesian_trajectories.constructGetCartesianPathRequest";
  ...
  // Initially empty
  test_case = "test_cartesian_trajectories.empty";

  check_and_emit(cache->countCartesianTrajectories(ROBOT_NAME) == 0, test_case, "Trajectory cache initially empty");
  ...
  //
  // Exact key match should have cache hit
  test_case = "test_cartesian_trajectories.put_second.fetch_matching_no_tolerance";
  ...
  //
  // Non-matching key should not have cache hit
  test_case = "test_cartesian_trajectories.put_second.fetch_non_matching_out_of_tolerance";
  ...
  test_case = "test_cartesian_trajectories.put_second.fetch_higher_fraction";
  ...
  test_case = "test_cartesian_trajectories.put_second.fetch_lower_fraction";
  ...
  //
  // With multiple trajectories in cache, fetches should be sorted accordingly
  test_case = "test_cartesian_trajectories.put_better.fetch_sorted";
  ...
  //
  // With multiple trajectories in cache, fetches should be sorted accordingly
  test_case = "test_cartesian_trajectories.put_different_req.fetch";
  ...
  //
  // Since we didn't populate anything, we should expect empty
  test_case = "test_cartesian_trajectories.different_robot.empty";
  std::string different_robot_name = "different_robot";
  ...
  check_and_emit(cache->countCartesianTrajectories(ROBOT_NAME) == 2, test_case,
...
int main(int argc, char** argv)
{
...

  {
    ...
    check_and_emit(cache->init(options), "init", "Cache init");

    test_getters_and_setters(cache);
