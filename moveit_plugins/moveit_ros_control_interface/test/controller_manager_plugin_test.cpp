...
std::shared_ptr<controller_manager_msgs::srv::ListControllers::Response> generate_response(int num_chained_controller, int rotation){
  ...
  for (int i = 0; i <= num_chained_controller; i++) {
    ...
    cs.claimed_interfaces = {chained_to_name + "/shoulder_pan_joint/position",
                             chained_to_name + "/shoulder_pan_joint/velocity",
                             chained_to_name + "/shoulder_lift_joint/position",
                             chained_to_name + "/shoulder_lift_joint/velocity",
                             chained_to_name + "/elbow_joint/position",
                             chained_to_name + "/elbow_joint/velocity",
                             chained_to_name + "/wrist_1_joint/position",
                             chained_to_name + "/wrist_1_joint/velocity",
                             chained_to_name + "/wrist_2_joint/position",
                             chained_to_name + "/wrist_2_joint/velocity",
                             chained_to_name + "/wrist_3_joint/position",
                             chained_to_name + "/wrist_3_joint/velocity"};
  ...
  cs.claimed_interfaces = {"shoulder_pan_joint/position",
  ...
  cs.claimed_interfaces = {"tcp_fts_sensor/force.x",
...
TEST(ControllerManagerPluginTest, FixChainedCntrollerInControllerListTest){
  ...
  std::vector<std::string> expected_interfaces = {"physical_interface0",
                                                  "physical_interface1",
  ...
  for (int rotation = 0; rotation < 4; rotation++){
    ...
    moveit_ros_control_interface::MoveItControllerManager::fix_chained_controllers(result);
